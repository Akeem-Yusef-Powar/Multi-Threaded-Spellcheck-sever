...
char logReports[maxRequest]={NULL,NULL,NULL};// results to log
...
int correct = 0;
    ...
    while(1)  // i hope this loop keeps the thread alive to consume next job.. IT DID!!
    {
        ...
        while(numLogs == maxRequest){
            pthread_cond_wait(&consumedLog,logLock);
        }
        if(correct == 1){
                       createLog(recvBuffer,correct);
                       pthread_cond_signal(&producedLog);
...
//----------------------------------------------------------
void createLog (char *what, int correctness)
{
    logReports[lPutPtr] = what;
    correctness[lPutPtr] = correctness;
    lPutPtr = lPutPtr + 1;
    numLogs = numLogs +1;
//get
// put to array
...
char zero = "Spelt Wrong\n";
char one = "Spelt Correct\n";

pthread_mutex_lock(&logLock);

while(numLogs==0 && logReports[lTakePtr]==NULL){
